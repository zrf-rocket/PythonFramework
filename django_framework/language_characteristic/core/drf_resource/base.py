...
from utils.thread_backed import ThreadPool
...
class Resource(six.with_metaclass(abc.ABCMeta, object)):
    ...
    def request(self, request_data, **kwargs):
        """
        执行请求，并对请求数据和返回数据进行数据校验
        """
        with tracer.start_as_current_span(f"drf_resource/{self.get_resource_name()}") as span:
            request_data = request_data or kwargs
